...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 1 or dataset == 'adc':
      self.names = adc_class_name
      print("come in class1 and adc")
    elif dataset == 'gta':
  ...
  def show_all_imgs(self, pause=False, time=0):
    ...
    if not self.ipynb:
      ...
      for i, v in self.imgs.items():
        ...
        cv2.imwrite("/data1/wjb/Centernet_mod/output/1.png",v)
  ...
  def save_img(self, path='./cache/debug/'):
    cv2.imwrite(path + '{}.png'.format(imgId), self.imgs[imgId])
    #print("path=",path)
    #cv2.imwrite(path+"/17790319373_bd19b24cfc_k.png",self.imgs['17790319373_bd19b24cfc_k'])
